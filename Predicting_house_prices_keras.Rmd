...
The naive model is based on the same example in the book <deep learning in R> by François Chollet with J. J. Allaire  
...
# train_X <- tidy_cross_val$train[[1]] %>% as_tibble() %>% select(-target) %>% as.matrix()
# test_X <- tidy_cross_val$test[[1]] %>% as_tibble() %>% select(-target) %>% as.matrix()
# train_y <- tidy_cross_val$train[[1]] %>% as_tibble() %>%  select(target) %>% as.matrix()
# test_y <- tidy_cross_val$test[[1]] %>% as_tibble() %>%  select(target) %>% as.matrix()
...
# tidy_cross_val$model_output %>% map(pluck,"history")
...
For the time series model we are going to train on the move in average monthly house price so the complete model can use this in predictions  
...
Scale numerics... (Remember that the test set must be scaled using the training scales otherwise you wouldn't be predicting using the same data scale ass trained)
...
However, note that when we are going to train the final model we will be using the keras model API to stack the 2 networks which means we wont be using the sequential model framework
...
This is the expected % error before we optimize the model and combine the LSTM network
...
So it should be OK if we use a model condidering a lag of 2 months.
...
# for (i in 1:epochs) {
#     model %>% keras::fit(x   = LSTM_input_full_model$LSTM_input,
#                   y          = LSTM_input_full_model$LSTM_target,
#                   batch_size = batch_size,
#                   epochs     = 1,
#                   verbose    = 1,
#                   shuffle    = FALSE)
...
# Make Predictions
# pred_out <- model %>% 
#     predict(LSTM_input_full_model$LSTM_input, batch_size = batch_size) %>%
#     .[,1] 
# Make Predictions
...
Now we need to back test the model on all the time slices
...
Now that we have validated and tested both our theoretical networks we want to combine these in order to predict the 
...
# predictions <- 
# 	model %>% 
# 	keras::predict_on_batch(list(inference_input = inference_train_X,
# LSTM_input = LSTM_input_full_model$LSTM_input))

# results <- 
# model %>% 
# keras::evaluate(list(inference_input = inference_train_X,
# LSTM_input = LSTM_input_full_model$LSTM_input), LSTM_input_full_model$LSTM_target, verbose = 1)
